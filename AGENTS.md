...
## grblHAL-core repository rules

This is the project's fork of the grblHAL core, on the `forgefirm` branch.
`grblHAL-glowforge` consumes it as a submodule at `src/grbl`.

### Rules specific to this repository

- **Minimum change.** Every commit here is a candidate for upstream. Carry
  the smallest change that solves the problem, in the surrounding style.
- **No rationale comments.** Do not explain in the code why this project
  needed a change. The reasoning goes in the commit message, which is what
  an upstream maintainer reads.
- **The goal is to need this fork less.** When an upstream release absorbs a
  commit, drop it and repoint. A pull request upstream is the developer's
